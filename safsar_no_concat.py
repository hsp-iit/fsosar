...
class SAFSAR(nn.Module):
    ...
    ### skeletons
    def encode_skeleton_seq(self, skel_seq):
        """
    ...
    def forward(self, support_set, support_labels, target_set, 
            batch_class_list=None, precomputed_context_features=None,
            support_skeleton=None, target_skeleton=None):
        ...
        if self.use_skeleton and (support_skeleton is not None):
            device = support_features.device
            B_sup = support_set.shape[0]  # numero video support dopo il reshape
            # (way*shot*seq_len, J, C) -> (B_sup, seq_len, J, C)
        ...
        #     support_global_logits = self.global_classification_layer(support_fused)   # (way, 768)
        #     query_global_logits   = self.global_classification_layer(query_fused)     # (n_queries, 768)
        # else:
        #     support_global_logits = 0. # We need to return something
        #     query_global_logits = 0. # We need to return something
        if self.use_l2_loss:
            # support: per-video
            support_global_logits = self.global_classification_layer(
                support_fused_per_video if support_fused_per_video is not None else support_features
            )  # (B_sup, n_train_classes)

            # query: fused
            query_global_logits = self.global_classification_layer(query_fused)  # (n_queries, n_train_classes)
        else:
