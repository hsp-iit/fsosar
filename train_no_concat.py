...
def main(rank, world_size, model_name, data_name, os_loss, port):
    ...
    while True:
        ...
        for elem in dataloader:
            ...
            # --- Skeleton tensors (allineati 1:1 ai frame) ---
            support_skeleton = elem["support_skeleton"].squeeze(0)   # (way*shot*seq_len, J, C)
            target_skeleton  = elem["target_skeleton"].squeeze(0)    # (n_queries*seq_len, J, C)
            unknown_skeleton = elem["unknown_skeleton"].squeeze(0)   # (n_unknown*seq_len, J, C)
            ...
            if os_loss != "softmax" or (not training and os_loss == "softmax") or config["eval_only"]:
                while True:
                    # concatena target + unknown
                    all_images    = torch.cat((target_set, unknown_set), 0).reshape(-1, config["seq_len"], *img_shape)
                    all_labels    = torch.cat((target_labels, torch.full_like(unknown_labels, -1)), 0)
                    all_unknowns  = torch.cat((torch.full_like(target_labels, 0), unknown_labels), 0)

                    all_skeletons = torch.cat((target_skeleton, unknown_skeleton), 0).reshape(-1, config["seq_len"], J, C)
                    ...
                    # taglia a maximum_queries
                    all_images    = torch.stack(all_images[:maximum_queries]).reshape(-1, config["seq_len"], *img_shape)
                    all_labels    = torch.stack(all_labels[:maximum_queries])
                    all_unknowns  = torch.stack(all_unknowns[:maximum_queries])
                    all_skeletons = torch.stack(all_skeletons[:maximum_queries]).reshape(-1, config["seq_len"], J, C)

                    # assicurati che ci sia almeno una query nota e una ignota
                    if (all_labels == -1).sum() > 0 and (all_labels != -1).sum() > 0:
                        break
            else:
                all_images    = target_set.reshape(-1, config["seq_len"], *img_shape)[:maximum_queries]
                all_labels    = target_labels[:maximum_queries]
                all_unknowns  = unknown_labels[:maximum_queries]
                all_skeletons = target_skeleton.reshape(-1, config["seq_len"], J, C)[:maximum_queries]
            ...
            logits = model(
                support_set, support_labels, all_images,
